...
{
    private readonly DeliverableRepository _repository;public DeliverablesController(DeliverableRepository repository)=>_repository=repository;
    [HttpGet]public async Task<IActionResult> Search([FromQuery]string? keyword,[FromQuery]int? departmentId,[FromQuery]int? typeId,[FromQuery]int? projectId,[FromQuery]string? status,[FromQuery]string? confidentiality,[FromQuery]string? sharePolicy,[FromQuery]int page=1,[FromQuery]int pageSize=20,CancellationToken ct=default)=>Ok(await _repository.SearchAsync(keyword,departmentId,typeId,projectId,status,confidentiality,sharePolicy,page,pageSize,ct));
