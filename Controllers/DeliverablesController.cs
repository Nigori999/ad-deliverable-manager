...
public sealed class DeliverablesController : ControllerBase
{
        ...
        CancellationToken ct = default)
    {
        ...
        return Ok(await _repository.SearchAsync(keyword, departmentId, typeId, projectId, status, confidentiality, sharePolicy, page, pageSize, ct, allowed));
    }
    ...
    public async Task<IActionResult> Get(int id, CancellationToken ct)
    {
        ...
        if (result is null) return NotFound(new { message = "交付物不存在。" });
        if (!await _permissions.HasPermissionAsync(User.GetUserId(), PermissionCatalog.DeliveryView, id, ct)) return Forbid();
        return Ok(result);
    }
    ...
    public async Task<IActionResult> AddVersion(int id, [FromBody] VersionCreateRequest request, CancellationToken ct)
    {
        try
        {
            await _repository.EnsureDirectVersionCreationAllowedAsync(id, true, ct);
            request.Operator = User.GetDisplayName();
            var versionId = await _repository.AddVersionWithOpenCyclePolicyAsync(id, request, ct);
    ...
    public async Task<IActionResult> SupplementVersion(int id, [FromBody] VersionCreateRequest request, CancellationToken ct)
    {
        if (!await _permissions.HasPermissionAsync(User.GetUserId(), PermissionCatalog.VersionSupplement, id, ct))
            return Forbid();

        try
        {
            if (!await _repository.HasFormalBaselineAsync(id, ct))
                return Conflict(new { message = "该交付物尚未形成正式基线，请使用正常的新增版本入口。" });

            await _repository.EnsureNoOpenVersionCycleAsync(id, ct);
            request.Operator = User.GetDisplayName();
            var versionId = await _repository.AddVersionWithOpenCyclePolicyAsync(id, request, ct);
            return Ok(new { id = versionId, message = "补录版本已创建为草稿，请继续按版本审批流程处理。" });
        }
        catch (ArgumentException ex) { return BadRequest(new { message = ex.Message }); }
        catch (InvalidOperationException ex) { return Conflict(new { message = ex.Message }); }
        catch (KeyNotFoundException ex) { return NotFound(new { message = ex.Message }); }
    ...
    public async Task<IActionResult> VersionAction(int versionId, string action, [FromBody] LifecycleActionRequest request, CancellationToken ct)
    {
        ...
        {
            request.Operator = User.GetDisplayName();
            var status = await _repository.TransitionVersionV073Async(versionId, normalized, request, ct);
    ...
    public async Task<IActionResult> Archive(int id, [FromBody] LifecycleActionRequest request, CancellationToken ct)
    {
        try
        {
            await _repository.ArchiveAsync(id, User.GetDisplayName(), request.Reason, ct);
