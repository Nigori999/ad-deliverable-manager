...
public sealed class UsersController : ControllerBase
{
    ...
    {
        try { var id = await _users.CreateAsync(request, User.GetDisplayName(), ct); return Ok(new { id, message = "用户已创建。" }); }
    ...
    {
        try { var ok = await _users.UpdateAsync(id, request, User.GetUserId(), User.GetDisplayName(), ct); return ok ? Ok(new { message = "用户已更新。" }) : Conflict(new { message = "用户信息已发生变化，请刷新后重试。" }); }
        catch (ArgumentException ex) { return BadRequest(new { message = ex.Message }); }
        catch (InvalidOperationException ex) { return Conflict(new { message = ex.Message }); }
    }
    ...
    {
        try { await _authUsers.ResetPasswordAsync(id, request, User.GetDisplayName(), ct); return Ok(new { message = "密码已重置。" }); }
        catch (ArgumentException ex) { return BadRequest(new { message = ex.Message }); }
    ...
    public async Task<IActionResult> Delete(int id, CancellationToken ct)
    {
        ...
        {
            await using var count = c.CreateCommand(); count.Transaction = tx; count.CommandText = "SELECT COUNT(*) FROM UserRoles ur JOIN Roles role ON role.Id=ur.RoleId JOIN Users u ON u.Id=ur.UserId WHERE role.Code='SYSTEM_ADMIN' AND role.IsEnabled=1 AND u.IsEnabled=1";
            if (Convert.ToInt32(await count.ExecuteScalarAsync(ct)) <= 1) return Conflict(new { message = "系统必须至少保留一个启用的系统管理员。" });
        }
