...
public sealed class AnalyticsController : ControllerBase
{
    ...
    public async Task<IActionResult> Completeness(CancellationToken ct)
    {
        ...
        var rows = await LoadMetadataRowsAsync(connection, ct);
        ...
        var projects = await LoadProjectsAsync(connection, ct);
        var types = await LoadTypesAsync(connection, ct);
        var categories = await LoadCategoriesAsync(connection, ct);
        var actualCoverage = await LoadActualCoverageAsync(connection, ct);
        ...
        var prdToFr = typeSet.Contains("PRD") && typeSet.Contains("FR") ? await LoadRelationTraceAsync(connection, "PRD", "FR", "DERIVES", ct) : TraceResult.Unavailable("当前启用配置未同时包含 PRD 与 FR");
        var frToTc = typeSet.Contains("FR") && typeSet.Contains("TC") ? await LoadRelationTraceAsync(connection, "FR", "TC", "VERIFIES", ct, bidirectional: true) : TraceResult.Unavailable("当前启用配置未同时包含 FR 与测试用例");
        var swpToTr = typeSet.Contains("SWP") && typeSet.Contains("TR") ? await LoadSwpTestReportTraceAsync(connection, ct) : TraceResult.Unavailable("当前启用配置未同时包含硬件软件包与测试报告");

        var pendingReview = await ScalarAsync(connection, "SELECT COUNT(*) FROM DeliverableVersions WHERE VersionStatus='IN_REVIEW'", ct);
        var pendingChanges = await ScalarAsync(connection, "SELECT COUNT(*) FROM ChangeRecords WHERE ChangeStatus NOT IN ('CLOSED','REJECTED')", ct);
    ...
    private async Task<List<CompletionItem>> LoadMetadataRowsAsync(SqliteConnection connection, CancellationToken ct)
    {
    ...
    private static async Task<List<ProjectItem>> LoadProjectsAsync(SqliteConnection connection,CancellationToken ct)
    {
        var list=new List<ProjectItem>();await using var cmd=connection.CreateCommand();cmd.CommandText="SELECT p.Id,p.ProjectCode,p.ProjectName FROM Projects p WHERE p.IsEnabled=1 ORDER BY p.ProjectCode";await using var reader=await cmd.ExecuteReaderAsync(ct);while(await reader.ReadAsync(ct))list.Add(new(reader.GetInt32(0),reader.GetString(1),reader.GetString(2)));return list;
    }
    private static async Task<List<TypeItem>> LoadTypesAsync(SqliteConnection connection,CancellationToken ct)
    {
        var list=new List<TypeItem>();await using var cmd=connection.CreateCommand();cmd.CommandText="SELECT t.Id,t.TypeCode,t.TypeName FROM DeliverableTypes t WHERE t.IsEnabled=1 ORDER BY t.SortOrder,t.TypeName";await using var reader=await cmd.ExecuteReaderAsync(ct);while(await reader.ReadAsync(ct))list.Add(new(reader.GetInt32(0),reader.GetString(1),reader.GetString(2)));return list;
    }
    private static async Task<List<CategoryItem>> LoadCategoriesAsync(SqliteConnection connection,CancellationToken ct)
    {
        var list=new List<CategoryItem>();await using var cmd=connection.CreateCommand();cmd.CommandText="SELECT t.TypeCode,c.CategoryCode,c.CategoryName FROM DeliverableCategories c JOIN DeliverableTypes t ON t.Id=c.DeliverableTypeId WHERE c.IsEnabled=1 AND t.IsEnabled=1 ORDER BY t.SortOrder,c.SortOrder,c.CategoryName";await using var reader=await cmd.ExecuteReaderAsync(ct);while(await reader.ReadAsync(ct))list.Add(new(reader.GetString(0),reader.GetString(1),reader.GetString(2)));return list;
    }
    private static async Task<List<CoverageItem>> LoadActualCoverageAsync(SqliteConnection connection,CancellationToken ct)
    {
        var list=new List<CoverageItem>();await using var cmd=connection.CreateCommand();cmd.CommandText="SELECT DISTINCT d.ProjectId,t.TypeCode,cat.CategoryCode FROM Deliverables d JOIN DeliverableTypes t ON t.Id=d.DeliverableTypeId JOIN DeliverableCategories cat ON cat.Id=d.CategoryId WHERE d.LifecycleStatus='ACTIVE'";await using var reader=await cmd.ExecuteReaderAsync(ct);while(await reader.ReadAsync(ct))list.Add(new(reader.GetInt32(0),reader.GetString(1),reader.GetString(2)));return list;
    }

    private static async Task<TraceResult> LoadRelationTraceAsync(SqliteConnection connection,string sourceType,string targetType,string relationType,CancellationToken ct,bool bidirectional=false)
    {
        var total=await ScalarAsync(connection,$"SELECT COUNT(*) FROM Deliverables d JOIN DeliverableTypes t ON t.Id=d.DeliverableTypeId WHERE d.LifecycleStatus='ACTIVE' AND t.TypeCode='{sourceType}'",ct);
        var linkedSql=bidirectional?$"SELECT COUNT(DISTINCT d.Id) FROM Deliverables d JOIN DeliverableTypes st ON st.Id=d.DeliverableTypeId JOIN DeliverableRelations r ON (r.SourceDeliverableId=d.Id OR r.TargetDeliverableId=d.Id) JOIN Deliverables o ON o.Id=CASE WHEN r.SourceDeliverableId=d.Id THEN r.TargetDeliverableId ELSE r.SourceDeliverableId END JOIN DeliverableTypes ot ON ot.Id=o.DeliverableTypeId WHERE d.LifecycleStatus='ACTIVE' AND o.LifecycleStatus='ACTIVE' AND st.TypeCode='{sourceType}' AND ot.TypeCode='{targetType}' AND r.RelationType='{relationType}'":$"SELECT COUNT(DISTINCT d.Id) FROM Deliverables d JOIN DeliverableTypes st ON st.Id=d.DeliverableTypeId JOIN DeliverableRelations r ON r.SourceDeliverableId=d.Id JOIN Deliverables o ON o.Id=r.TargetDeliverableId JOIN DeliverableTypes ot ON ot.Id=o.DeliverableTypeId WHERE d.LifecycleStatus='ACTIVE' AND o.LifecycleStatus='ACTIVE' AND st.TypeCode='{sourceType}' AND ot.TypeCode='{targetType}' AND r.RelationType='{relationType}'";
        var linked=await ScalarAsync(connection,linkedSql,ct);return new TraceResult(true,total,linked,Percent(linked,total),null,[]);
    }

    private static async Task<TraceResult> LoadSwpTestReportTraceAsync(SqliteConnection connection,CancellationToken ct)
    {
        var details=new List<SwpTestReportDetail>();await using var cmd=connection.CreateCommand();cmd.CommandText="""
            SELECT p.Id,p.ProjectCode,p.ProjectName,cat.CategoryCode,cat.CategoryName,
                   CASE WHEN EXISTS(
                       ...
                       WHERE tr.ProjectId=d.ProjectId AND tr.LifecycleStatus='ACTIVE' AND tt.TypeCode='TR' AND tc.CategoryCode=cat.CategoryCode
    ...
    private static async Task<int> ScalarAsync(SqliteConnection connection,string sql,CancellationToken ct){await using var command=connection.CreateCommand();command.CommandText=sql;return Convert.ToInt32(await command.ExecuteScalarAsync(ct));}
