...
public sealed class DeliverySchedulesController : ControllerBase
{
    ...
    public async Task<IActionResult> Get([FromQuery] int? projectId, CancellationToken ct)
    {
        ...
        await using (var command = connection.CreateCommand())
        {
            ...
            while (await reader.ReadAsync(ct))
            {
                var category = new CategoryOption(
                    ...
                    reader.GetInt32(3), reader.GetString(4), reader.GetString(5), reader.GetInt32(6));
                if (await _permissions.HasCreateScopeAsync(userId, PermissionCatalog.DeliveryScheduleView,
                        category.DepartmentId, selectedProjectId.Value, category.TypeId, ct))
        ...
        await using (var command = connection.CreateCommand())
        {
            ...
            while (await reader.ReadAsync(ct))
            {
                ...
                items.Add(new ScheduleRow(
                    ...
                    typeId, reader.GetString(6), reader.GetString(7), planned, actual, reader.GetInt32(11), status.Code, status.Days));
            }
        }
        ...
        {
            total = items.Count,
            available = categories.Count(x => !plannedCategoryIds.Contains(x.Id)),
    ...
    public async Task<IActionResult> Create([FromBody] CreatePlansRequest request, CancellationToken ct)
    {
        ...
        foreach (var item in normalized)
        {
            ...
            categoryCommand.CommandText = """
                ...
                WHERE c.Id=$id AND c.IsEnabled=1 AND t.IsEnabled=1;
                """;
            ...
            if (!await _permissions.HasCreateScopeAsync(userId, PermissionCatalog.DeliveryScheduleEdit,
                    departmentId, request.ProjectId, typeId, ct))
            ...
            duplicate.Parameters.AddWithValue("$projectId", request.ProjectId);
            duplicate.Parameters.AddWithValue("$categoryId", item.CategoryId);
            if (Convert.ToInt32(await duplicate.ExecuteScalarAsync(ct)) > 0)
                return Conflict(new { message = $"类别“{categoryName}”已存在交付计划，请直接修改日期。" });
        ...
        foreach (var item in validated)
        {
            ...
            insert.CommandText = """
                INSERT INTO ProjectDeliverablePlans(ProjectId,CategoryId,PlannedDeliveryDate,CreatedBy,CreatedAt,UpdatedBy,UpdatedAt,Revision)
                VALUES($projectId,$categoryId,$date,$operator,$now,$operator,$now,1)
                RETURNING Id;
                """;
            insert.Parameters.AddWithValue("$projectId", request.ProjectId);
            insert.Parameters.AddWithValue("$categoryId", item.CategoryId);
            insert.Parameters.AddWithValue("$date", item.Date);
            insert.Parameters.AddWithValue("$operator", operatorName);
            insert.Parameters.AddWithValue("$now", now);
            var planId = Convert.ToInt32(await insert.ExecuteScalarAsync(ct));
            await InsertAuditAsync(connection, transaction, planId, "CREATE", operatorName,
                $"新增车型交付计划：{item.CategoryName}，计划日期 {item.Date}", now, ct);
        }
        await transaction.CommitAsync(ct);
        return Ok(new { created = validated.Count });
    }
    ...
    public async Task<IActionResult> UpdateDates([FromBody] UpdatePlanDatesRequest request, CancellationToken ct)
    {
        ...
        foreach (var plan in plans)
        {
            ...
            update.CommandText = "UPDATE ProjectDeliverablePlans SET PlannedDeliveryDate=$date,UpdatedBy=$operator,UpdatedAt=$now,Revision=Revision+1 WHERE Id=$id";
            update.Parameters.AddWithValue("$date", date);
            update.Parameters.AddWithValue("$operator", operatorName);
            update.Parameters.AddWithValue("$now", now);
            update.Parameters.AddWithValue("$id", plan.Id);
            await update.ExecuteNonQueryAsync(ct);
            await InsertAuditAsync(connection, transaction, plan.Id, "UPDATE", operatorName,
                $"修改车型交付计划：{plan.CategoryName}，计划日期 {date}", now, ct);
        }
        await transaction.CommitAsync(ct);
        return Ok(new { updated = plans.Count, plannedDeliveryDate = date });
    }
    ...
    public async Task<IActionResult> Delete([FromBody] DeletePlansRequest request, CancellationToken ct)
    {
        ...
        foreach (var plan in plans)
        {
            ...
            delete.CommandText = "DELETE FROM ProjectDeliverablePlans WHERE Id=$id";
            delete.Parameters.AddWithValue("$id", plan.Id);
            await delete.ExecuteNonQueryAsync(ct);
            await InsertAuditAsync(connection, transaction, plan.Id, "DELETE", operatorName,
                $"删除车型交付计划：{plan.CategoryName}", now, ct);
        }
        await transaction.CommitAsync(ct);
        return Ok(new { deleted = plans.Count });
    }

    private async Task<List<PlanAccessRow>> ValidatePlanAccessAsync(
        Microsoft.Data.Sqlite.SqliteConnection connection, int[] ids, int userId, string permissionCode, CancellationToken ct)
    {
        ...
        foreach (var id in ids)
        {
            await using var command = connection.CreateCommand();
            command.CommandText = """
                SELECT p.Id,p.ProjectId,c.CategoryName,t.Id,t.DepartmentId
                FROM ProjectDeliverablePlans p
                JOIN DeliverableCategories c ON c.Id=p.CategoryId
                JOIN DeliverableTypes t ON t.Id=c.DeliverableTypeId
                WHERE p.Id=$id;
            ...
            await using var reader = await command.ExecuteReaderAsync(ct);
            if (!await reader.ReadAsync(ct)) throw new KeyNotFoundException("交付计划不存在或已被删除。");
            var row = new PlanAccessRow(reader.GetInt32(0), reader.GetInt32(1), reader.GetString(2), reader.GetInt32(3), reader.GetInt32(4));
            await reader.DisposeAsync();
            if (!await _permissions.HasCreateScopeAsync(userId, permissionCode, row.DepartmentId, row.ProjectId, row.TypeId, ct))
                throw new UnauthorizedAccessException($"当前账号无权修改交付计划“{row.CategoryName}”。");
            result.Add(row);
        }
        return result;
    }
    ...
    {
        if (string.IsNullOrWhiteSpace(value) || !DateOnly.TryParseExact(value.Trim(), "yyyy-MM-dd", CultureInfo.InvariantCulture, DateTimeStyles.None, out var date))
            throw new ArgumentException("计划交付日期格式无效。");
    ...
    private static async Task InsertAuditAsync(
        ...
        int planId, string action, string operatorName, string summary, string now, CancellationToken ct)
    {
        await using var audit = connection.CreateCommand();
        audit.Transaction = transaction;
        audit.CommandText = "INSERT INTO AuditLogs(EntityType,EntityId,ActionType,Operator,Summary,DetailJson,CreatedAt) VALUES('ProjectDeliverablePlan',$id,$action,$operator,$summary,'{}',$now)";
        audit.Parameters.AddWithValue("$id", planId);
    ...
    public sealed class CreatePlansRequest
    {
        public int ProjectId { get; set; }
        public CreatePlanItem[] Items { get; set; } = [];
        public string Operator { get; set; } = "系统用户";
    }

    public sealed class CreatePlanItem
    {
        public int CategoryId { get; set; }
        public string PlannedDeliveryDate { get; set; } = "";
    }

    public sealed class UpdatePlanDatesRequest
    {
    ...
    public sealed class DeletePlansRequest
    {
        public int[] PlanIds { get; set; } = [];
        public string Operator { get; set; } = "系统用户";
    }
    ...
    private sealed record CategoryOption(int Id, string Code, string Name, int TypeId, string TypeCode, string TypeName, int DepartmentId);
    private sealed record ScheduleRow(
        int Id, int ProjectId, int CategoryId, string CategoryCode, string CategoryName,
        int TypeId, string TypeCode, string TypeName, string PlannedDeliveryDate,
        string? ActualDeliveryDate, int MatchedDeliverables, string Status, int Days);
