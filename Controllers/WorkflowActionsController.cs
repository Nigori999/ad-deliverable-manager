...
public sealed class WorkflowActionsController : ControllerBase
{
    ...
    public Task<IActionResult> SubmitVersionReview(int versionId, [FromBody] LifecycleActionRequest request, CancellationToken cancellationToken)
    ...
    public Task<IActionResult> ReturnVersionToDraft(int versionId, [FromBody] LifecycleActionRequest request, CancellationToken cancellationToken)
        => RunVersionActionAsync(versionId, "return-draft", request, true, cancellationToken);

    [HttpPost("versions/{versionId:int}/approve")]
    public Task<IActionResult> ApproveVersion(int versionId, [FromBody] LifecycleActionRequest request, CancellationToken cancellationToken)
        => RunVersionActionAsync(versionId, "approve", request, true, cancellationToken);

    [HttpPost("versions/{versionId:int}/release")]
    public Task<IActionResult> ReleaseVersion(int versionId, [FromBody] LifecycleActionRequest request, CancellationToken cancellationToken)
        => RunVersionActionAsync(versionId, "release", request, true, cancellationToken);

    [HttpPost("versions/{versionId:int}/deprecate")]
    public Task<IActionResult> DeprecateVersion(int versionId, [FromBody] LifecycleActionRequest request, CancellationToken cancellationToken)
        => RunVersionActionAsync(versionId, "deprecate", request, true, cancellationToken);

    [HttpPost("changes/{id:int}/approve")]
    public Task<IActionResult> ApproveChange(int id, [FromBody] ChangeActionRequest request, CancellationToken cancellationToken)
        => RunChangeActionAsync(id, "approve", request, cancellationToken);

    [HttpPost("changes/{id:int}/reject")]
    public Task<IActionResult> RejectChange(int id, [FromBody] ChangeActionRequest request, CancellationToken cancellationToken)
        => RunChangeActionAsync(id, "reject", request, cancellationToken);

    [HttpPost("changes/{id:int}/start")]
    public Task<IActionResult> StartChange(int id, [FromBody] ChangeActionRequest request, CancellationToken cancellationToken)
        => RunChangeActionAsync(id, "start", request, cancellationToken);

    [HttpPost("changes/{id:int}/verify")]
    public Task<IActionResult> VerifyChange(int id, [FromBody] ChangeActionRequest request, CancellationToken cancellationToken)
        => RunChangeActionAsync(id, "verify", request, cancellationToken);

    [HttpPost("changes/{id:int}/close")]
    public Task<IActionResult> CloseChange(int id, [FromBody] ChangeActionRequest request, CancellationToken cancellationToken)
        => RunChangeActionAsync(id, "close", request, cancellationToken);

    private async Task<IActionResult> RunVersionActionAsync(int versionId, string action, LifecycleActionRequest request, bool reasonRequired, CancellationToken cancellationToken)
    {
        if (reasonRequired && string.IsNullOrWhiteSpace(request.Reason)) return BadRequest(new { message = "该操作必须填写处理意见。" });
        try
        {
    ...
    private async Task<IActionResult> RunChangeActionAsync(int id, string action, ChangeActionRequest request, CancellationToken cancellationToken)
    {
        ...
        if ((action is "approve" or "reject") && string.IsNullOrWhiteSpace(request.Opinion)) return BadRequest(new { message = "批准或驳回时必须填写评审意见。" });
        ...
        if (action is "verify" or "close")
        {
            ...
            if (!string.Equals(reader.GetString(0), transition.Required, StringComparison.Ordinal)) return Conflict(new { message = "当前状态不允许执行该操作，请刷新后重试。" });
            if (reader.IsDBNull(1)) return BadRequest(new { message = "请先创建并关联变更后版本，再提交验证。" });
        ...
        if (await command.ExecuteNonQueryAsync(cancellationToken) == 0) return Conflict(new { message = "当前状态不允许执行该操作，请刷新后重试。" });

        await using var audit = connection.CreateCommand();
        audit.Transaction = transaction;
        audit.CommandText = "INSERT INTO AuditLogs(EntityType,EntityId,ActionType,Operator,Summary,CreatedAt) VALUES('Change',$id,$action,$operator,$summary,$now);";
        audit.Parameters.AddValue("$id", id); audit.Parameters.AddValue("$action", action.ToUpperInvariant()); audit.Parameters.AddValue("$operator", request.Operator); audit.Parameters.AddValue("$summary", $"变更状态：{transition.Required} → {transition.Target}"); audit.Parameters.AddValue("$now", now);
