...
public sealed class MasterDataController : ControllerBase
{
    ...
    public async Task<IActionResult> Get(CancellationToken cancellationToken)
    {
        ...
        var departments=await ReadAsync("SELECT Id, DepartmentCode, DepartmentName, 0 FROM Departments WHERE IsEnabled=1 ORDER BY SortOrder");var projects=await ReadAsync("SELECT Id, ProjectCode, ProjectName, 0 FROM Projects WHERE IsEnabled=1 ORDER BY ProjectCode");var types=await ReadAsync("SELECT Id, TypeCode, TypeName, DepartmentId, HasHardwareFields FROM DeliverableTypes WHERE IsEnabled=1 ORDER BY SortOrder",true,true);
