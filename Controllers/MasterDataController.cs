...
public sealed class MasterDataController : ControllerBase
{
    ...
    public async Task<IActionResult> Get(CancellationToken cancellationToken)
    {
        ...
        return Ok(new{departments,projects,types,confidentialityLevels=new[]{new{code="PUBLIC",name="公开"},new{code="INTERNAL",name="内部"},new{code="CONFIDENTIAL",name="秘密"},new{code="STRICTLY_CONFIDENTIAL",name="机密"}},sharePolicies=new[]{new{code="ALLOWED",name="允许对外分享"},new{code="APPROVAL_REQUIRED",name="审批后允许"},new{code="PROHIBITED",name="禁止对外分享"}},hardwareCategories=new[]{"前视摄像头","周视摄像头","角雷达","激光雷达","毫米波雷达","超声波雷达","智驾域控制器"}});
    }
    ...
    {
        if(string.IsNullOrWhiteSpace(request.ProjectCode)||string.IsNullOrWhiteSpace(request.ProjectName))return BadRequest(new{message="项目编码和项目名称不能为空。"});
        await using var connection=await _database.OpenConnectionAsync(cancellationToken);await using var command=connection.CreateCommand();command.CommandText="UPDATE Projects SET ProjectCode=$code,ProjectName=$name,VehicleModel=$vehicle,PlatformName=$platform,UpdatedAt=$now,Revision=Revision+1 WHERE Id=$id AND Revision=$revision AND IsEnabled=1";command.Parameters.AddValue("$code",request.ProjectCode.Trim().ToUpperInvariant());command.Parameters.AddValue("$name",request.ProjectName.Trim());command.Parameters.AddValue("$vehicle",request.VehicleModel);command.Parameters.AddValue("$platform",request.PlatformName);command.Parameters.AddValue("$now",DateTime.UtcNow.ToString("O"));command.Parameters.AddValue("$id",id);command.Parameters.AddValue("$revision",request.Revision);try{if(await command.ExecuteNonQueryAsync(cancellationToken)==0)return Conflict(new{message="项目不存在、已删除或已被其他人修改，请刷新后重试。"});return Ok(new{message="项目已更新。"});}catch(SqliteException ex)when(ex.SqliteErrorCode==19){return Conflict(new{message="项目编码已存在。"});}
    }

    [HttpDelete("projects/{id:int}")]
    public async Task<IActionResult> DeleteProject(int id,CancellationToken cancellationToken)
    {
        await using var connection=await _database.OpenConnectionAsync(cancellationToken);await using var check=connection.CreateCommand();check.CommandText="SELECT COUNT(*) FROM Deliverables WHERE ProjectId=$id";check.Parameters.AddValue("$id",id);var used=Convert.ToInt32(await check.ExecuteScalarAsync(cancellationToken));if(used>0)return Conflict(new{message=$"该项目已被 {used} 个交付物引用，不能删除。请先处理关联数据。"});await using var command=connection.CreateCommand();command.CommandText="UPDATE Projects SET IsEnabled=0,UpdatedAt=$now,Revision=Revision+1 WHERE Id=$id AND IsEnabled=1";command.Parameters.AddValue("$id",id);command.Parameters.AddValue("$now",DateTime.UtcNow.ToString("O"));if(await command.ExecuteNonQueryAsync(cancellationToken)==0)return NotFound(new{message="项目不存在或已删除。"});return Ok(new{message="项目已删除。"});
    }
}
