...
builder.Services.AddAuthentication(CookieAuthenticationDefaults.AuthenticationScheme).AddCookie(options =>
{
    ...
    options.Events.OnRedirectToLogin = c => { c.Response.StatusCode = 401; return c.Response.WriteAsJsonAsync(new { message = "登录已失效，请重新登录。" }); };
    options.Events.OnRedirectToAccessDenied = c => { c.Response.StatusCode = 403; return c.Response.WriteAsJsonAsync(new { message = "当前账号没有执行该操作的权限。" }); };
});
...
builder.Services.AddScoped<JiraReviewRepository>();
...
var permissionSchema = app.Services.GetRequiredService<PermissionSchemaService>();
await permissionSchema.EnsureAsync();
...
app.Use(async (context, next) =>
{
    ...
    var isAnonymous = context.Request.Path.StartsWithSegments("/internal/auth/login") ||
    ...
    if (context.User.Identity?.IsAuthenticated == true && isInternal)
    {
        ...
        var current = await users.FindByIdAsync(context.User.GetUserId(), context.RequestAborted);
        if (current is null || !current.IsEnabled)
        {
            await context.SignOutAsync(CookieAuthenticationDefaults.AuthenticationScheme);
            context.Response.StatusCode = 401;
            await context.Response.WriteAsJsonAsync(new { message = "账号已停用，请联系管理员。" });
            return;
        }
        var allowedPasswordPath = context.Request.Path.StartsWithSegments("/internal/auth/change-password") ||
                                  ...
                                  context.Request.Path.StartsWithSegments("/internal/auth/status");
        if (current.MustChangePassword && !allowedPasswordPath)
        {
...
if (app.Configuration.GetValue("Application:AutoOpenBrowser", true))
    ...
    {
        try { Process.Start(new ProcessStartInfo { FileName = $"http://localhost:{port}", UseShellExecute = true }); } catch { }
    });
await app.RunAsync();
