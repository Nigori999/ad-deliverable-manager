...
public sealed class PermissionRoutingMiddleware
{
    ...
    public PermissionRoutingMiddleware(RequestDelegate next)=>_next=next;

    public async Task InvokeAsync(HttpContext context)
    {
        var path=context.Request.Path.Value??"";
        if(!path.StartsWith("/internal",StringComparison.OrdinalIgnoreCase)){await _next(context);return;}
        if(path.StartsWith("/internal/auth/",StringComparison.OrdinalIgnoreCase)){await _next(context);return;}
        if(path.Equals("/internal/system/health",StringComparison.OrdinalIgnoreCase)){await _next(context);return;}
        ...
        var rule=ResolveRule(context.Request.Method,path);
        if(rule is null){context.Response.StatusCode=403;await context.Response.WriteAsJsonAsync(new{message="该操作尚未配置权限策略，请联系管理员。"});return;}
        var service=context.RequestServices.GetRequiredService<PermissionService>();
        ...
        if(!deliverableId.HasValue&&int.TryParse(GetSegmentAfter(path,"version-details"),out var detailVersion))deliverableId=await service.ResolveDeliverableIdByVersionAsync(detailVersion,context.RequestAborted);
        if(!deliverableId.HasValue&&int.TryParse(GetSegmentAfter(path,"versions"),out var versionId))deliverableId=await service.ResolveDeliverableIdByVersionAsync(versionId,context.RequestAborted);
        if(!deliverableId.HasValue){var versionIndex=path.IndexOf("/versions/",StringComparison.OrdinalIgnoreCase);if(versionIndex>=0){var text=path[(versionIndex+10)..].Split('/')[0];if(int.TryParse(text,out var vId))deliverableId=await service.ResolveDeliverableIdByVersionAsync(vId,context.RequestAborted);}}
        if(!deliverableId.HasValue){var changeIndex=path.IndexOf("/changes/",StringComparison.OrdinalIgnoreCase);if(changeIndex>=0){var rest=path[(changeIndex+9)..].Split('/');if(rest.Length>0&&int.TryParse(rest[0],out var changeId))deliverableId=await ResolveChangeDeliverableIdAsync(changeId,context.RequestServices,context.RequestAborted);}}

        if(!await service.HasPermissionAsync(context.User.GetUserId(),rule.Permission,rule.Node,deliverableId,context.RequestAborted))
        {context.Response.StatusCode=403;await context.Response.WriteAsJsonAsync(new{message=rule.Message??"当前账号没有执行该操作的权限。"});return;}
        await _next(context);
    }
    ...
    private static Rule? ResolveRule(string method,string path)
    {
        ...
        if(path.StartsWith("/internal/version-details",StringComparison.OrdinalIgnoreCase))return new(PermissionCatalog.DeliveryView,null,"当前角色没有查看版本详情的权限。");
        ...
        {
            if(method=="GET")return new(PermissionCatalog.DeliveryView,null,"当前角色没有查看交付物的权限。");
            if(method=="POST"&&path.Contains("/archive",StringComparison.OrdinalIgnoreCase))return new(PermissionCatalog.DeliveryArchive,null,"当前角色没有归档交付物的权限。");
            if(method=="POST"&&path.Contains("/versions",StringComparison.OrdinalIgnoreCase))return new(PermissionCatalog.VersionCreate,null,"当前角色没有创建版本的权限。");
            if(method=="POST")return new(PermissionCatalog.DeliveryCreate,null,"当前角色没有新增交付物的权限。");
            if(method=="PUT")return new(PermissionCatalog.DeliveryEdit,null,"当前角色没有编辑交付物的权限。");
            if(method=="DELETE")return new(PermissionCatalog.DeliveryArchive,null,"当前角色没有删除或归档交付物的权限。");
            if(path.Contains("/versions/",StringComparison.OrdinalIgnoreCase)){var action=path.Split('/').LastOrDefault();return VersionActionRule(action);}
        }
