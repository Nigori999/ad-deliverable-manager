async function renderDashboard() {
  ...
  content.innerHTML = `
    ...
    <section class="card" style="margin:18px 0"><div class="card-head"><div><h3>交付节点预警</h3><p class="muted section-note">预警来源于车型交付计划；实际交付日期自动匹配同车型、同类别台账中的最早首版创建时间。</p></div>${hasPermission('DELIVERY_SCHEDULE_VIEW')?'<a class="btn btn-light btn-sm" href="#/delivery-schedules">查看交付计划</a>':''}</div><div class="card-body">
      ...
      <div class="recent-list">${(data.deliveryRisks||[]).length?(data.deliveryRisks||[]).map(x=>{const status=x.status==='OVERDUE_UNDELIVERED'?`已延期${Math.max(0,Number(x.days||0))}天`:(Number(x.days||0)===0?'今天到期':`${Math.max(0,Number(x.days||0))}天后到期`);const inner=`<div><strong>${esc(x.name)}</strong><small>${esc(x.vehicleModel||x.projectName)} · ${esc(x.typeName)} · 计划 ${esc(x.plannedDeliveryDate)}</small></div><span class="badge ${x.status==='OVERDUE_UNDELIVERED'?'deprecated':'in_review'}">${esc(status)}</span>`;return hasPermission('DELIVERY_SCHEDULE_VIEW')?`<a class="recent-row" href="#/delivery-schedules/${x.status}/${x.projectId}">${inner}</a>`:`<div class="recent-row">${inner}</div>`;}).join(''):'<div class="empty">当前没有即将到期或延期未交付的车型交付计划。</div>'}</div>
    </div></section>
