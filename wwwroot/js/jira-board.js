...
function mergeJiraAnalyses(entries,cutoffDate){
  ...
  const funnel=stages.map(([code,name],order)=>{const count=issues.filter(x=>x.maxReachedStageOrder>=order).length,previous=order?issues.filter(x=>x.maxReachedStageOrder>=order-1).length:issues.length;return{code,name:code==='new'?'创建':name,order,count,previousConversion:jiraPercentValue(count,previous),dropFromPrevious:order?Math.max(0,previous-count):0};});
...
function jiraFunnelChart(items) {
  ...
  const points = items.map((item,index) => {
    ...
    return `<g class="jira-funnel-layer" data-jira-stage="${esc(item.code)}" tabindex="0" role="button" aria-label="${esc(item.name)}累计到达${item.count}项，环节转化率${jiraPercent(item.previousConversion)}"><rect class="hit" x="20" y="${y}" width="960" height="${rowHeight}" fill="transparent"></rect>${item.count?`<polygon points="${center-top/2},${y+2} ${center+top/2},${y+2} ${center+bottom/2},${y+rowHeight-3} ${center-bottom/2},${y+rowHeight-3}" fill="${jiraStageColors[item.code]}"></polygon>`:`<line class="zero" x1="${center-20}" y1="${y+rowHeight/2}" x2="${center+20}" y2="${y+rowHeight/2}"></line>`}<g class="label"><rect x="${center-132}" y="${y+12}" width="264" height="58" rx="9"></rect><text x="${center}" y="${y+31}" text-anchor="middle">${esc(item.name)} · ${jiraNumber(item.count)}项</text><text class="sub" x="${center}" y="${y+49}" text-anchor="middle">环节转化 ${jiraPercent(item.previousConversion)}</text><text class="sub loss" x="${center}" y="${y+65}" text-anchor="middle">${index?'较上一阶段流失 '+jiraNumber(item.dropFromPrevious)+' 项':'全部问题基准'}</text></g></g>`;
  }).join('');
