async function renderAnalytics() {
  setPage('完整度分析', '管理层全局查看车型交付完整度与上下游协同闭环');
  ...
  content.innerHTML = `
    <div class="ux-page-intro analytics-intro"><div><strong>完整度 = 车型交付是否齐全 + 上下游协同是否闭环</strong><span>该报表面向管理层展示全局统计，不按部门、项目或交付物类型数据范围裁剪。车型视角按“项目 × 交付物类型 × 启用类别”动态检查；协同视角关注 PRD→FR、FR→测试用例，以及硬件类别的软件包→测试报告验收闭环。</span></div></div>
    ...
    <section class="analytics-view-section" id="analytics-vehicle"><div class="analytics-section-head"><div><span class="analytics-section-kicker">01 · 车型视角</span><h3>车型交付物完整度</h3><p>回答“这个车型该有的交付物是否齐全”。期望项直接读取基础设置中各交付物类型启用的类别，不在代码中写死。</p></div><span class="analytics-section-badge">${projectIssues.length?`${projectIssues.length} 项缺失`:'全部完整'}</span></div>
      <div class="analytics-project-grid">${(data.projectCompleteness||[]).length?(data.projectCompleteness||[]).map(project=>projectCompletenessCard(project)).join(''):'<div class="empty">当前没有可分析的车型/项目</div>'}</div>
    </section>
