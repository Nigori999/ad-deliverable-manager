...
""";
            ...
            while (await r.ReadAsync(ct))
            {
                hardware.Add(new { id = r.GetInt32(0), name = r.GetString(1), version = r.GetString(2), hardwareCategory = r.IsDBNull(3) ? "" : r.GetString(3), hardwareModel = r.IsDBNull(4) ? "" : r.GetString(4) });
            }
        }
...
""";
            ...
            while (await r.ReadAsync(ct))
            {
                documents.Add(new { id = r.GetInt32(0), name = r.GetString(1), version = r.GetString(2), typeCode = r.GetString(3), typeName = r.GetString(4) });
            }
        }
    ...
    public async Task<int> CreateAsync(ProductBaselineCreateRequest request, string operatorName, CancellationToken ct = default)
    {
        ...
        check.CommandText = "SELECT COUNT(*) FROM ProductBaselines WHERE ProductName=$name AND InternalVersion='V1.0.0'";
        check.Parameters.AddWithValue("$name", request.ProductName.Trim());
        if (Convert.ToInt32(await check.ExecuteScalarAsync(ct)) > 0) throw new InvalidOperationException("该产品已存在V1.0.0，请从已发布基线复制创建后续版本。");
...
VALUES($name,'V1.0.0','DRAFT',$description,$vehicles,$odd,$capabilities,$by,$now,$now,1);
...
""";
    ...
    public async Task UpdateDraftAsync(int id, ProductBaselineUpdateRequest request, string operatorName, CancellationToken ct = default)
    {
        ...
        {
            cmd.Transaction = tx;
            cmd.CommandText = "UPDATE ProductBaselines SET ProductName=$name,Description=$description,VehicleModels=$vehicles,Odd=$odd,Capabilities=$capabilities,UpdatedAt=$now,Revision=Revision+1 WHERE Id=$id AND Revision=$revision";
            cmd.Parameters.AddWithValue("$name", request.ProductName.Trim());
            cmd.Parameters.AddWithValue("$description", request.Description?.Trim() ?? "");
            cmd.Parameters.AddWithValue("$vehicles", request.VehicleModels?.Trim() ?? "");
            cmd.Parameters.AddWithValue("$odd", request.Odd?.Trim() ?? "");
        ...
        {
            await using var add = c.CreateCommand();
            add.Transaction = tx;
            ...
            add.Parameters.AddWithValue("$category", item.HardwareCategory.Trim());
            add.Parameters.AddWithValue("$model", item.HardwareModel?.Trim() ?? "");
            add.Parameters.AddWithValue("$version", item.SoftwareVersionId);
        ...
        foreach (var item in request.Deliverables.DistinctBy(x => (x.RoleCode, x.VersionId)))
        {
            ...
            add.Parameters.AddWithValue("$id", id);
            add.Parameters.AddWithValue("$role", item.RoleCode.Trim().ToUpperInvariant());
            add.Parameters.AddWithValue("$version", item.VersionId);
            await add.ExecuteNonQueryAsync(ct);
        }
    ...
    public async Task PublishAsync(int id, int revision, string operatorName, CancellationToken ct = default)
    {
        ...
        await using var cmd = c.CreateCommand();
        cmd.Transaction = tx;
        cmd.CommandText = "UPDATE ProductBaselines SET VersionStatus='RELEASED',ReleaseDate=$date,UpdatedAt=$date,Revision=Revision+1 WHERE Id=$id AND Revision=$revision";
...
VALUES($name,$version,'DRAFT',$description,$vehicles,$odd,$capabilities,$based,$by,$now,$now,1);
SELECT last_insert_rowid();
""";
        cmd.Parameters.AddWithValue("$name", source.ProductName);
        cmd.Parameters.AddWithValue("$version", next);
        cmd.Parameters.AddWithValue("$description", source.Description);
        cmd.Parameters.AddWithValue("$vehicles", source.VehicleModels);
        cmd.Parameters.AddWithValue("$odd", source.Odd);
        cmd.Parameters.AddWithValue("$capabilities", source.Capabilities);
        cmd.Parameters.AddWithValue("$based", source.Id);
        cmd.Parameters.AddWithValue("$by", operatorName);
        cmd.Parameters.AddWithValue("$now", DateTime.UtcNow.ToString("O"));
        var id = Convert.ToInt32(await cmd.ExecuteScalarAsync(ct));
    ...
    public async Task ApplyChangeAsync(int id, ProductBaselineChangeRequest request, string operatorName, CancellationToken ct = default)
    {
        ...
        {
            cmd.Transaction = tx;
            cmd.CommandText = "UPDATE ProductBaselines SET Description=$description,VehicleModels=$vehicles,Odd=$odd,Capabilities=$capabilities,UpdatedAt=$now,Revision=Revision+1 WHERE Id=$id AND Revision=$revision AND VersionStatus='RELEASED'";
            cmd.Parameters.AddWithValue("$description", request.Description?.Trim() ?? "");
            cmd.Parameters.AddWithValue("$vehicles", request.VehicleModels?.Trim() ?? "");
            cmd.Parameters.AddWithValue("$odd", request.Odd?.Trim() ?? "");
            cmd.Parameters.AddWithValue("$capabilities", request.Capabilities?.Trim() ?? "");
        ...
        await using (var log = c.CreateCommand())
        {
            ...
            log.Parameters.AddWithValue("$reason", request.ChangeReason.Trim());
            log.Parameters.AddWithValue("$description", request.Description?.Trim() ?? "");
            log.Parameters.AddWithValue("$before", before);
            log.Parameters.AddWithValue("$after", after);
            log.Parameters.AddWithValue("$by", operatorName);
    ...
    {
        if (string.IsNullOrWhiteSpace(name) || name.Trim().Length > 80) throw new ArgumentException("产品名称不能为空且不超过80字。");
    }

    private static void ValidateComponents(List<ProductBaselineHardwareRequest> hardware)
    {
        if (hardware.Count == 0) throw new ArgumentException("至少配置一项硬件及其对应软件包。");
        if (hardware.Any(x => string.IsNullOrWhiteSpace(x.HardwareCategory) || x.SoftwareVersionId <= 0)) throw new ArgumentException("每项硬件都必须配置类别和对应的软件包版本。");
    ...
    private static async Task EnsureDraftAsync(SqliteConnection c, SqliteTransaction tx, int id, int revision, CancellationToken ct)
    {
        await using var cmd = c.CreateCommand();
        cmd.Transaction = tx;
        cmd.CommandText = "SELECT VersionStatus FROM ProductBaselines WHERE Id=$id AND Revision=$revision";
        cmd.Parameters.AddWithValue("$id", id);
        cmd.Parameters.AddWithValue("$revision", revision);
        var status = await cmd.ExecuteScalarAsync(ct) as string;
        if (status is null) throw new InvalidOperationException("基线不存在或已被其他人修改，请刷新后重试。");
        if (!string.Equals(status, "DRAFT", StringComparison.OrdinalIgnoreCase)) throw new InvalidOperationException("已发布基线不能直接修改。");
    }
    ...
    private static async Task ValidatePublishAsync(SqliteConnection c, SqliteTransaction tx, int id, CancellationToken ct)
    {
        ...
        count.CommandText = "SELECT COUNT(*) FROM ProductBaselineHardware WHERE BaselineId=$id";
        ...
        await using var badHardware = c.CreateCommand();
        badHardware.Transaction = tx;
        badHardware.CommandText = """
SELECT COUNT(*)
FROM ProductBaselineHardware h
...
""";
        ...
        await using var badDocuments = c.CreateCommand();
        badDocuments.Transaction = tx;
        badDocuments.CommandText = """
SELECT COUNT(*)
FROM ProductBaselineDeliverables b
JOIN DeliverableVersions v ON v.Id=b.VersionId
JOIN Deliverables d ON d.Id=v.DeliverableId
JOIN DeliverableTypes t ON t.Id=d.DeliverableTypeId
WHERE b.BaselineId=$id AND (b.RoleCode NOT IN ('PRD','FR','TC','TR') OR t.TypeCode<>b.RoleCode OR v.VersionStatus NOT IN ('RELEASED','SUPERSEDED'))
""";
        badDocuments.Parameters.AddWithValue("$id", id);
        if (Convert.ToInt32(await badDocuments.ExecuteScalarAsync(ct)) > 0) throw new ArgumentException("基线中的辅助交付物必须引用对应类型的已正式发布或已替代版本。");
    }

    private static async Task<BaselineSnapshot?> ReadBaselineAsync(SqliteConnection c, int id, CancellationToken ct)
    {
        await using var cmd = c.CreateCommand();
        cmd.CommandText = "SELECT Id,ProductName,InternalVersion,VersionStatus,Description,VehicleModels,Odd,Capabilities,BasedOnBaselineId,CreatedBy,CreatedAt,ReleaseDate,UpdatedAt,Revision FROM ProductBaselines WHERE Id=$id";
        cmd.Parameters.AddWithValue("$id", id);
        await using var r = await cmd.ExecuteReaderAsync(ct);
        ...
        return new BaselineSnapshot(
            ...
            r.IsDBNull(6) ? "" : r.GetString(6), r.IsDBNull(7) ? "" : r.GetString(7),
            r.IsDBNull(8) ? null : r.GetInt32(8), r.GetString(9), r.GetString(10),
            r.IsDBNull(11) ? null : r.GetString(11), r.GetString(12), r.GetInt32(13));
    }

    private static async Task<List<object>> ReadHardwareAsync(SqliteConnection c, int id, CancellationToken ct)
    {
        await using var cmd = c.CreateCommand();
        cmd.CommandText = "SELECT h.HardwareCategory,h.HardwareModel,h.SoftwareVersionId,d.UnifiedName,v.InternalVersion FROM ProductBaselineHardware h JOIN DeliverableVersions v ON v.Id=h.SoftwareVersionId JOIN Deliverables d ON d.Id=v.DeliverableId WHERE h.BaselineId=$id ORDER BY h.HardwareCategory";
        cmd.Parameters.AddWithValue("$id", id);
        var rows = new List<object>();
        await using var r = await cmd.ExecuteReaderAsync(ct);
        while (await r.ReadAsync(ct)) rows.Add(new { hardwareCategory = r.GetString(0), hardwareModel = r.IsDBNull(1) ? "" : r.GetString(1), softwareVersionId = r.GetInt32(2), softwareName = r.GetString(3), softwareVersion = r.GetString(4) });
        return rows;
    }

    private static async Task<List<object>> ReadDeliverablesAsync(SqliteConnection c, int id, CancellationToken ct)
    {
        await using var cmd = c.CreateCommand();
        cmd.CommandText = "SELECT b.RoleCode,b.VersionId,d.UnifiedName,v.InternalVersion,t.TypeName FROM ProductBaselineDeliverables b JOIN DeliverableVersions v ON v.Id=b.VersionId JOIN Deliverables d ON d.Id=v.DeliverableId JOIN DeliverableTypes t ON t.Id=d.DeliverableTypeId WHERE b.BaselineId=$id ORDER BY b.RoleCode,d.UnifiedName";
        cmd.Parameters.AddWithValue("$id", id);
        var rows = new List<object>();
        await using var r = await cmd.ExecuteReaderAsync(ct);
        while (await r.ReadAsync(ct)) rows.Add(new { roleCode = r.GetString(0), versionId = r.GetInt32(1), name = r.GetString(2), version = r.GetString(3), typeName = r.GetString(4) });
        return rows;
    }

    private static async Task<List<object>> ReadChangesAsync(SqliteConnection c, int id, CancellationToken ct)
    {
        await using var cmd = c.CreateCommand();
        cmd.CommandText = "SELECT Id,ChangeReason,Description,Operator,CreatedAt FROM ProductBaselineChanges WHERE BaselineId=$id ORDER BY CreatedAt DESC";
        cmd.Parameters.AddWithValue("$id", id);
        var rows = new List<object>();
        await using var r = await cmd.ExecuteReaderAsync(ct);
        while (await r.ReadAsync(ct)) rows.Add(new { id = r.GetInt32(0), changeReason = r.GetString(1), description = r.IsDBNull(2) ? "" : r.GetString(2), operatorName = r.GetString(3), createdAt = r.GetString(4) });
        return rows;
    }
    ...
    {
        var parts = current.TrimStart('V').Split('.').Select(int.Parse).ToArray();
        if (parts.Length != 3) throw new InvalidOperationException("产品基线版本号必须符合V主.次.修订格式。");
        if (type == 0) { parts[0]++; parts[1] = 0; parts[2] = 0; }
        else if (type == 1) { parts[1]++; parts[2] = 0; }
        else parts[2]++;
        var candidate = $"V{parts[0]}.{parts[1]}.{parts[2]}";
        await using var cmd = c.CreateCommand();
        cmd.CommandText = "SELECT COUNT(*) FROM ProductBaselines WHERE ProductName=$name AND InternalVersion=$version";
        cmd.Parameters.AddWithValue("$name", productName);
        cmd.Parameters.AddWithValue("$version", candidate);
        if (Convert.ToInt32(await cmd.ExecuteScalarAsync(ct)) > 0) throw new InvalidOperationException($"版本{candidate}已存在，请选择其他版本类型。");
        return candidate;
    }
}
