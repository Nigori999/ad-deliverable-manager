...
public sealed partial class DeliverableRepository
{
    ...
    public async Task<object> SearchAsync(string? keyword,int? departmentId,int? typeId,int? projectId,string? status,string? confidentiality,string? sharePolicy,int page,int pageSize,CancellationToken cancellationToken,IReadOnlyList<int>? allowedDeliverableIds=null,int? categoryId=null)
    {
        ...
        if(!string.IsNullOrWhiteSpace(keyword)){where.Append(" AND (d.DeliverableCode LIKE $keyword OR d.UnifiedName LIKE $keyword OR d.ObjectCode LIKE $keyword OR d.ResponsiblePerson LIKE $keyword OR cat.CategoryName LIKE $keyword) ");parameters.Add(("$keyword",$"%{keyword.Trim()}%"));}
        ...
        await using var command=connection.CreateCommand();command.CommandText=$"""
            SELECT d.Id,d.DeliverableCode,d.UnifiedName,dep.DepartmentName,t.TypeName,cat.Id,cat.CategoryCode,cat.CategoryName,p.ProjectName,d.ObjectCode,d.ResponsiblePerson,d.DefaultConfidentiality,d.DefaultSharePolicy,d.LifecycleStatus,d.UpdatedAt,d.Revision,v.InternalVersion,v.VersionStatus,v.ServerPath,
        ...
        var items=new List<object>();await using var reader=await command.ExecuteReaderAsync(cancellationToken);while(await reader.ReadAsync(cancellationToken))items.Add(new{id=reader.GetInt32(0),code=reader.GetString(1),name=reader.GetString(2),department=reader.GetString(3),type=reader.GetString(4),categoryId=reader.GetInt32(5),categoryCode=reader.GetString(6),category=reader.GetString(7),project=reader.GetString(8),objectCode=reader.GetString(9),responsiblePerson=reader.GetString(10),confidentiality=reader.GetString(11),sharePolicy=reader.GetString(12),lifecycleStatus=reader.GetString(13),updatedAt=reader.GetString(14),revision=reader.GetInt32(15),currentVersion=reader.GetNullableString(16),versionStatus=reader.GetNullableString(17),serverPath=reader.GetNullableString(18),canDeleteDraft=reader.GetInt32(19)==1});return new{items,total,page,pageSize};
    }
